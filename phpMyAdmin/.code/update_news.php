...
//アップロード済画像
$del_pic1 = '';
$sql_test = "SELECT `img1_name` FROM `deetrading_co_jp_news` WHERE `No`='$No'";
$result_test = $connection->query($sql_test);
while($rows1=$result_test->fetch(PDO::FETCH_ASSOC)){
    $del_pic1 = $rows1["img1_name"];
}

$del_pic2 = '';
$sql_test = "SELECT `img2_name` FROM `deetrading_co_jp_news` WHERE `No`='$No'";
$result_test2 = $connection->query($sql_test);
while($rows2=$result_test2->fetch(PDO::FETCH_ASSOC)){
    $del_pic2 = $rows2["img2_name"];
}

// moving file
if($sendTo != 'jp' && $del_pic1){
    $org_pic1 = 'upload/upload1/'.$del_pic1;
    $move_pic1 = 'com/upload/upload1/'.$del_pic1;
...
if($sendTo != 'jp' && $del_pic2){
$org_pic2 = 'upload/upload2/'.$del_pic2;
$move_pic2 = 'com/upload/upload2/'.$del_pic2;
...
if($uploaded_img1 != $del_pic1 && $del_pic1){
    ...
    $sql = "UPDATE `deetrading_co_jp_news` SET `img1_name`='',`image1`='' WHERE `No`='$No' ";
    $result = $connection->query($sql);
}
}

if($uploaded_img2 || $sendTo == "com"){
if($uploaded_img2 != $del_pic2 && $del_pic2){
    ...
    $sql = "UPDATE `deetrading_co_jp_news` SET `img2_name`='',`image2`='' WHERE `No`='$No' ";
...
if($sendTo == 'com'){
    $sql1 = "DELETE from `deetrading_co_jp_news` WHERE `No`= '$No'";
    $sql_sendTo = "INSERT INTO deetrading_com_news (No, date, title, information, img1_name, image1, img2_name, image2, youtube_video, other_link, id, role, sendTo) VALUES ('0', '$date', '$title', '$information', '$name', '$image', '$name2', '$image2', '$video', '$other_link', '$loginUser', '$loginUserRole', 'com')";

    //fire query to save entries and check it with if statement
    $result_sendTo = $connection1->query($sql_sendTo);
    $rs = $connection->query($sql1);

} else if($sendTo == 'both') {
    // This below line is a code to Send form entries to database
    if ($image && (strlen($image2) == 0)){
        $sql = "UPDATE `deetrading_co_jp_news` SET `date`='$date',`title`='$title',`information`='$information',`img1_name`='$name',`image1`='$image',`other_link`='$other_link',`youtube_video`='$video' WHERE `No`='$No' ";
    } else if ($image2 && (strlen($image) == 0)){
        $sql = "UPDATE `deetrading_co_jp_news` SET `date`='$date',`title`='$title',`information`='$information',`img2_name`='$name2',`image2`='$image2',`other_link`='$other_link',`youtube_video`='$video' WHERE `No`='$No' ";
    } else if($image && $image2){
        $sql = "UPDATE `deetrading_co_jp_news` SET `date`='$date',`title`='$title',`information`='$information',`img1_name`='$name',`image1`='$image',`img2_name`='$name2',`image2`='$image2',`other_link`='$other_link',`youtube_video`='$video' WHERE `No`='$No' ";
    } else {
        $sql = "UPDATE `deetrading_co_jp_news` SET `date`='$date',`title`='$title',`information`='$information',`other_link`='$other_link',`youtube_video`='$video' WHERE `No`='$No' ";
    }
...
if($rs)
{
    ...
    header("location:/newslist?css=site1&" . time());// redirects to all records page
